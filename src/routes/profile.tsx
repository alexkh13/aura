          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                Switch between wardrobes or create a new one
              </p>
            </div>
          </div>
...
function ProfileCard({ profile, isActive, canDelete, onSelect, onDelete }: ProfileCardProps) {
  const { data: items } = useItems(profile.id)
  const { data: outfits } = useOutfits(profile.id)
    ...
    >
      ...
      <button onClick={onSelect} className="w-full text-left pr-10">
        ...
        <div className="flex gap-4 text-sm">
          ...
          <div className="flex items-center gap-1.5 text-gray-600 dark:text-gray-400">
            <span className="font-medium text-gray-900 dark:text-gray-100">{outfitCount}</span>
            outfits
          </div>
        </div>
      </button>
