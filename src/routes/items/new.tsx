...
import { useCreateItem, useDeleteItem, useUpdateItem, useItems } from '@/hooks/useData'
import { useProfile } from '@/hooks/useProfile'
import { extractGarmentsFromImage, isGeminiConfigured, generateOutfitDetails, type AIGeneratedItem } from '@/services/ai'
...
function AddNewItemPage() {
  ...
  const { data: allItems } = useItems(activeProfile?.id)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (allSaved) {
        // All items saved, show outfit creation dialog
        setTimeout(() => setShowOutfitDialog(true), 1000)
      } else {
  ...
  const handleRemove = async () => {
    ...
    if (newItems.length === 0) {
      // If no items left, navigate to wardrobe
  ...
  const handleCreateOutfit = async () => {
    ...
    // Navigate to outfit creation with pre-selected items and AI data
    navigate({
      to: '/outfits/new',
      state: {
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                title={currentItem.saved ? 'Remove item' : 'Discard item'}
              >
              ...
              <button
                onClick={handleSave}
                disabled={createItem.isPending || updateItem.isPending || showSuccess}
              ...
              {hasGeminiKey && (
                <div className="mb-4 p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg border border-purple-200 dark:border-purple-800">
                  <div className="flex items-center gap-2 text-sm text-purple-700 dark:text-purple-300">
                    ...
                    <span>AI will suggest outfit details</span>
                  </div>
                </div>
              )}
